...
STAFF_NAMES = [
    ("HK-01", "Sita Kulkarni", StaffRole.HOUSEKEEPING, 1),
    ("HK-02", "Ramesh Pawar", StaffRole.HOUSEKEEPING, 1),
    ("HK-03", "Fatima Shaikh", StaffRole.HOUSEKEEPING, 2),
    ("HK-04", "Joseph D'Souza", StaffRole.HOUSEKEEPING, 2),
    ("HK-05", "Kavita More", StaffRole.HOUSEKEEPING, 3),
    ("HK-06", "Imran Qureshi", StaffRole.HOUSEKEEPING, 3),
    ("IN-01", "Anita Desai", StaffRole.INSPECTOR, 2),
    ("IN-02", "Sunil Joshi", StaffRole.INSPECTOR, 3),
    ("MT-01", "Prakash Naik", StaffRole.MAINTENANCE, 1),
    ("MT-02", "Lina George", StaffRole.MAINTENANCE, 2),
]
...
class HotelState:
    def __init__(self) -> None:
        ...
        # Hotel minutes advanced per real second. 1.0 = readable demo; 3.0 = pitch mode.
        self.sim_minutes_per_second = 1.0
        self.seed()
    ...
    def seed(self) -> None:
        ...
        for sid, name, role, floor in STAFF_NAMES:
            st = Staff(id=sid, name=name, role=role, floor=floor, minutes_worked=float((hash(sid) % 40)))
            self.staff[sid] = st
