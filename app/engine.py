...
from app.models import Room, RoomStatus, RoomType, Staff, StaffRole, StaffSeniority, Task, TaskKind
    ...
    for i, s in enumerate(free):
        for j, r in enumerate(pending):
            ...
            if kind == TaskKind.CLEAN and r.room_type == RoomType.SUITE and s.quality_score < 0.9:
                skill_pen = 10.0
            senior = s.seniority == StaffSeniority.SENIOR
            seniority_pen = -6.0 if senior else 3.5
            if r.vip and not senior:
                seniority_pen += 12.0  # VIP rooms prefer senior specialists
            c = eta + floor_pen + load_pen + vip_boost + arrival_boost + skill_pen + seniority_pen
            cost[i, j] = c
            bits = []
            bits.append("Senior" if senior else "Junior")
            if r.vip:
                bits.append("VIP" if senior else "VIP — junior penalty")
