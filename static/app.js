  ...
  $("staffCards").innerHTML = staff
    .map((s) => {
      ...
      return `<div class="scard">
        ...
        <div class="meta">${s.id} · ${s.role}</div>
...
$("speedBtns").addEventListener("click", async (ev) => {
  const btn = ev.target.closest("button");
  if (!btn) return;
  await fetch("/api/speed", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ minutes_per_second: Number(btn.dataset.min) }),
  });
});

function syncSpeedButtons(minutes) {
  $("speedBtns").querySelectorAll("button").forEach((b) => {
    b.classList.toggle("on", Number(b.dataset.min) === Number(minutes));
  });
}
...
function paint(state) {
  ...
  syncSpeedButtons(state.kpis.sim_minutes_per_second);
